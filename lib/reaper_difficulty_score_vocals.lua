...
--
-- SEPARATE FROM reaper_difficulty_score.lua ON PURPOSE. The gem scorer measures a hand
-- moving across lanes; this measures a voice moving through pitch classes, and the two
-- share no factor definitions at all. It does share four small span/percentile helpers,
-- which is why they are globals over there rather than duplicated here:
--   NormalizeSpans, TotalSpanSeconds, EventsInSegments, Percentile, SpanOverlapSeconds
--
-- LOAD reaper_difficulty_score.lua FIRST, and not only for those helpers: the tail of
-- this file APPENDS the vocal columns to SCORE_FACTOR_KEYS, which that file defines. So
-- it must also load before anything that reads that list - the CSV writer, the analysis,
-- and the model exporter all drive off it.
--
-- Pure: no `r`, no `ctx`, no `S`. Everything arrives as plain tables, so the tests drive
-- it directly with no REAPER project. Shared by the calibration harness and the general
-- helper's difficulty suggestion - see the header of reaper_difficulty_score.lua for why
-- there is exactly one copy. dev/calibration/difficulty_score_vocals.lua is a one-line
-- loader kept so the calibration entry points need no edits.
--
