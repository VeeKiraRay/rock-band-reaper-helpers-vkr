-- All tooltip strings (TIPS table)
...
TIPS = {
    ...
    snap_enabled =
        "After detection, nudge each note boundary to the nearest peak energy\n" ..
        "rise (start) or fall (end) within the snap window.\n\n" ..
        "Useful when Auto-tune sets the right note count but boundaries are\n" ..
        "slightly early or late due to RMS window quantisation.",

    snap_window_ms =
        "How far (ms) from each detected boundary to search for a sharper\n" ..
        "energy transition. Larger values allow more movement but may snap\n" ..
        "to the wrong onset if notes are close together.",

    draft_snap_window_ms =
        "How far (ms) from each draft note boundary to search for the nearest\n" ..
        "energy onset or offset. 100 ms is a good default - enough to correct\n" ..
        "rough placement without jumping to a neighbouring syllable.",

    snap_draft =
        "Read existing notes from the MIDI destination track and snap each\n" ..
        "start/end to the nearest energy onset within the snap window.\n\n" ..
        "Draw a rough note count first (right count, approximate position),\n" ..
        "then click here to lock boundaries to the audio. Pitch is assigned\n" ..
        "using the source configured in the Pitch tab.\n\n" ..
    ...
    -- General tab
    show_wip_tabs =
        "Show work-in-progress tabs (Note Placement).\n\n" ..
